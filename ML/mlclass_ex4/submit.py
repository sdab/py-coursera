...
def output(part_id, auxstring):
    X = 3 * sin(arange(1,31)).reshape(3,10,order='F')
    Xm = sin(arange(1,33)).reshape(16,2,order='F') / 5
    ym = 1 + arange(1,17) % 4
    t1 = sin(arange(1, 24, 2.).reshape(4,3,order='F'))
    t2 = cos(arange(1, 40, 2.).reshape(4,5,order='F'))
    t = hstack((t1.ravel(order='F'),t2.ravel(order='F')))
    ...
    if part_id == 1:
        J, _ = func(t, 2, 4, 4, Xm, ym, 0)
        return sprintf('%0.5f ', J)
    elif part_id == 2:
        J, _ = func(t, 2, 4, 4, Xm, ym, 1.5)
        return sprintf('%0.5f ', J)
    elif part_id == 3:
        return sprintf('%0.5f ', func(X))
    elif part_id == 4:
        J, grad = func(t, 2, 4, 4, Xm, ym, 0)
        return sprintf('%0.5f ', J) + sprintf('%0.5f ', grad)
    elif part_id == 5:
